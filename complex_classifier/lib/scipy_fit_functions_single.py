    ...
    for num_try in range(num_tries): #retry fit num_tries times (with different random p0)
        try:
            if pole_class == 0:
                ...
                p0_new = get_p0(p0)         
                params_tmp, _ = curve_fit(objective_1r_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_1r_jac_single, xtol=xtol0, method=method) if with_bounds else \
                              curve_fit(objective_1r_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_1r_jac_single, xtol=xtol0, method=method)

            elif pole_class == 1:
                lower = [re_min, im_min, -coeff_re_max, -coeff_im_max]
                upper = [re_max, im_max, coeff_re_max, coeff_im_max]   
                p0_new = get_p0(p0)          
                params_tmp, _ = curve_fit(objective_1c_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_1c_jac_single, xtol=xtol1, method=method) if with_bounds else \
                              curve_fit(objective_1c_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_1c_jac_single, xtol=xtol1, method=method)

            elif pole_class == 2:
                lower = [re_min, -coeff_re_max, re_min, -coeff_re_max]
                upper = [re_max, coeff_re_max, re_max, coeff_re_max]   
                p0_new = get_p0(p0)
                params_tmp, _ = curve_fit(objective_2r_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_2r_jac_single, xtol=xtol2, method=method) if with_bounds else \
                              curve_fit(objective_2r_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_2r_jac_single, xtol=xtol2, method=method)
                ...
                p0_new = get_p0(p0)
                params_tmp, _ = curve_fit(objective_1r1c_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_1r1c_jac_single, xtol=xtol3, method=method) if with_bounds else \
                              curve_fit(objective_1r1c_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_1r1c_jac_single, xtol=xtol3, method=method)
                ...
                p0_new = get_p0(p0)
                params_tmp, _ = curve_fit(objective_2c_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_2c_jac_single, xtol=xtol4, method=method) if with_bounds else \
                              curve_fit(objective_2c_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_2c_jac_single, xtol=xtol4, method=method)
                ...
                p0_new = get_p0(p0)
                params_tmp, _ = curve_fit(objective_3r_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_3r_jac_single, xtol=xtol5, method=method) if with_bounds else \
                              curve_fit(objective_3r_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_3r_jac_single, xtol=xtol5, method=method)
                ...
                p0_new = get_p0(p0)
                params_tmp, _ = curve_fit(objective_2r1c_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_2r1c_jac_single, xtol=xtol6, method=method) if with_bounds else \
                              curve_fit(objective_2r1c_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_2r1c_jac_single, xtol=xtol6, method=method)
                ...
                p0_new = get_p0(p0)
                params_tmp, _ = curve_fit(objective_1r2c_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_1r2c_jac_single, xtol=xtol7, method=method) if with_bounds else \
                              curve_fit(objective_1r2c_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_1r2c_jac_single, xtol=xtol7, method=method)
                ...
                p0_new = get_p0(p0)
                params_tmp, _ = curve_fit(objective_3c_single, grid_x, data_y, maxfev=maxfev, bounds=(lower, upper), p0=p0_new, jac=objective_3c_jac_single, xtol=xtol8, method=method) if with_bounds else \
                              curve_fit(objective_3c_single, grid_x, data_y, maxfev=maxfev, p0=p0_new, jac=objective_3c_jac_single, xtol=xtol8, method=method)
        except:
            print('Fit failed!')
